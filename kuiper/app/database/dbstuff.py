...
DB_USER_PASSWORD = app.config['DB_USER_PASSWORD']
...
try:
    if (DB_USER != '') and (DB_USER_PASSWORD != ''):
        MClient = MongoClient(f'mongodb://{DB_USER}:{DB_USER_PASSWORD}@#{DB_IP}:{DB_PORT}/{DB_NAME}?authSource={DB_NAME}&authMechanism=SCRAM-SHA-256&ssl=true')
    else:
        MClient = MongoClient(f'{DB_IP}:{DB_PORT}')        
except Exception as e:
    MClient = None
