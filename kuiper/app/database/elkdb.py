...
def get_es():
    #TODO should use app.config instead of os.getenv(...)?
...
class ES_DB:
    ...
    # ================================ get max results window
    # get the setting for maximum number of records to be retrived from elasticsearch
    # TODO delete function, it's not used anywhere.

    def _get_max_fields_num(self,  indx):
        settings = self.es_db.indices.get_settings(index=indx)
        settings = settings[indx]['settings']['index']
    ...
    def multiqueries(self, case_id, bodies):
        index_name = self._get_index_name(case_id)
        request_header = json.dumps({'index': index_name})
        requests = []
        for b in bodies:
    ...
    def query(self, case_id, body, count=3, scroll=None):
        count -= 1
        index_name = self._get_index_name(case_id)
        body["track_total_hits"] = True
        logger.logger(level=logger.DEBUG, type="elasticsearch",
                      message="Query to index ["+index_name+"]", reason=json.dumps(body))
        filter_path = ['hits.hits._source.Data',
                       'hits.total.value', 'aggregations.*.buckets']
        try:
            #search_res = self.es_db.search(index=indexname,body=body , filter_path=filter_path)
            search_res = self.es_db.search(
                index=index_name, body=body, scroll=scroll)
            return [True, search_res]
        except elasticsearch.RequestError as e:
            reason = e.info['error']['reason']
            logger.logger(level=logger.WARNING, type="elasticsearch",
                          message="Query ["+index_name+"] failed [RequestError]", reason=reason)
                ...
                for shard in e.info['error']['failed_shards']:
                    ...
                    if shard_reason.startswith("Text fields are not optimised for operations that require per-document field data like aggregations and sorting, so these operations are disabled by default"):
                        if "sort" in body.keys():
                            ...
                            logger.logger(level=logger.INFO, type="elasticsearch",
                                          message="Query ["+index_name+"], the sort is not a sortable field, try using sub-field .keyword")
                            return self.query(index_name, body, count)

                    # if the reason is the result has too many fields
                    match = re.match(
                        'field expansion (for \[.*\] )?matches too many fields, limit: ([0-9]+), got: ([0-9]+)', shard_reason)
                    if match is not None:
                        ...
                        inc = self.es_db.indices.put_settings(
                            index=index_name, body='{ "index" : { "query": { "default_field" : '+str(max_field_num)+'} } }')
                        if inc["acknowledged"]:
                            logger.logger(level=logger.INFO, type="elasticsearch",
                                          message="Query ["+index_name+"] max query fields number increased " + str(max_field_num))
                            if count != 0:
                                return self.query(index_name, body, count)
                            else:
                            ...
                            logger.logger(level=logger.ERROR, type="elasticsearch",
                                          message="Query ["+index_name+"] Failed increasing the result window")
                            continue

                    # if the result window is too large, increase the window
                    match = re.match(
                        'Result window is too large, from \+ size must be less than or equal to: \[([0-9]+)\] but was \[([0-9]+)\].*', shard_reason)
                    if match is not None:
                        ...
                        inc = self.es_db.indices.put_settings(
                            index=error_index, body='{ "index" : { "max_result_window" : ' + str(max_result_window) + ' } }')
                        if inc["acknowledged"]:
                            logger.logger(level=logger.INFO, type="elasticsearch",
                                          message="Query ["+index_name+"] result window increased to " + str(self._get_max_result_window(index_name)))
                            if count != 0:
                                return self.query(index_name, body, count)
                            else:
                        ...
                        logger.logger(level=logger.ERROR, type="elasticsearch",
                                      message="Query ["+index_name+"] failed [RequestError]", reason=shard_reason)
            else:
                logger.logger(level=logger.ERROR, type="elasticsearch",
                              message="Query ["+index_name+"] failed [RequestError]", reason=json.dumps(e.info))
            res = [False, reason]
        except elasticsearch.ConnectionError as e:
            logger.logger(level=logger.ERROR, type="elasticsearch",
                          message="Query ["+index_name+"] failed [ConnectionError]", reason=e.info)
            res = [False, 'Failed to connect to elasticsearch']
        except elasticsearch.TransportError as e:
            reason = str(e)
            logger.logger(level=logger.ERROR, type="elasticsearch",
                          message="Query ["+index_name+"] failed [TransportError]", reason=reason)
            logger.logger(level=logger.ERROR, type="elasticsearch",
                          message="Query ["+index_name+"] failed [TransportError]", reason=json.dumps(e.info))
            res = [False, reason]
        except elasticsearch.ElasticsearchException as e:
            reason = str(e)
            logger.logger(level=logger.ERROR, type="elasticsearch",
                          message="Query ["+index_name+"] failed [ElasticsearchException]", reason=reason)
            logger.logger(level=logger.ERROR, type="elasticsearch",
    ...
    def query_scroll(self, scroll_id, scroll="5m"):
        logger.logger(level=logger.DEBUG, type="elasticsearch",
                      message="Scroll Query to index", reason=scroll_id)
        try:
    ...
    def bulk_queue_push(self, data, case_id, chunk_size=500):
        index_name = self._get_index_name(case_id)        
        bulk_queue = []
        ...
        for d in data:
            ...
            di['_source']['Data'] = d['Data']
            if 'data_source' in d and d['data_source'] is not None:
                di['_source']['data_source'] = d['data_source']
            if 'data_type' in d and d['data_type'] is not None:
                di['_source']['data_type'] = d['data_type']
            if 'data_type' in d and d['data_type'] is not None:
                di['_source']['data_type'] = d['data_type']
            if 'data_path' in d and d['data_path'] is not None:
                di['_source']['data_path'] = d['data_path']
            if 'machine' in d and d['machine'] is not None:
                di['_source']['machine'] = d['machine']

            bulk_queue.append(di)

        logger.logger(level=logger.DEBUG, type="elasticsearch",
                      message="Index ["+index_name+"]: Pushing ["+str(len(bulk_queue))+"] records")

        push_es = self._bulk_to_elasticsearch(bulk_queue, index_name, chunk_size)
        if push_es[0]:
            logger.logger(level=logger.INFO, type="elasticsearch", message="Index ["+index_name+"]: Pushed ["+str(
                len(bulk_queue) - len(push_es[2]))+"] records successfully")
            return [True, "Pushed ["+str(len(bulk_queue))+"] records", push_es[2], push_es[3]]
        else:
            logger.logger(level=logger.ERROR, type="elasticsearch",
                          message="Index ["+index_name+"]: Failed pusheing ["+str(len(bulk_queue))+"] records", reason=push_es[1])
    ...
    def _bulk_to_elasticsearch(self,  bulk_queue, indx, chunk_size):
        ...
        try:
            ...
            logger.logger(level=logger.DEBUG, type="elasticsearch", message="Index ["+indx+"]: bulk push to ES, default chunk["+str(
                chunk_size)+"]: ", reason="number of records: " + str(len(bulk_queue)))
            ...
            for ok, item in helpers.parallel_bulk(self.es_db, bulk_queue, chunk_size=chunk_size, raise_on_error=False, raise_on_exception=False):
                if not ok:
                    ...
                    logger.logger(level=logger.WARNING, type="elasticsearch",
                                  message="Index ["+indx+"]: Failed pushing record: ", reason=str(item))
            ...
            if len(failed):
                logger.logger(level=logger.WARNING, type="elasticsearch",
                              message="Index ["+indx+"]: Failed pushing ["+str(len(failed))+"] records, try to fix the issue")
                # get origin data from ID
                for data in bulk_queue:
                    try:
                        errors[data['_id']]['index']['data'] = data['_source']
                        logger.logger(level=logger.DEBUG, type="elasticsearch",
                                      message="Index ["+indx+"]: get data for failed record ["+data['_id']+"]", reason=str(errors[data['_id']]))
                    except:
                        # if record not in the errors list, continue
                        continue
                    logger.logger(level=logger.WARNING, type="elasticsearch",
                                  message="Index ["+indx+"]: Failed pushing record: ", reason=str(data['_id']))

                fixed_errors, nonfixed_errors = self._bulk_to_elasticsearch_fix_errors(
                    indx, errors)
                failed = nonfixed_errors
                if len(fixed_errors):
                    logger.logger(level=logger.DEBUG, type="elasticsearch",
                                  message="Index ["+indx+"]: fixed issue of ["+str(len(fixed_errors))+"] records, retry to push it")
                    repush_failed_errors = self._bulk_to_elasticsearch(
        ...
        # if connection timeout to elasticsearch occurred
            ...
            logger.logger(level=logger.WARNING, type="elasticsearch",
                          message="Index ["+indx+"]: Failed to push the records, retry again", reason="Connection to Elasticsearch timeout")
            return self._bulk_to_elasticsearch(bulk_queue, indx, chunk_size)

        except Exception as e:
            logger.logger(level=logger.ERROR, type="elasticsearch",
                          message="Failed pushing the records, unexpected error", reason=str(e))
    ...
    def _bulk_to_elasticsearch_fix_errors(self, indx, errors):
        logger.logger(level=logger.WARNING, type="elasticsearch", message="Index ["+indx+"]: Failed pushing ["+str(
            len(errors))+"] records [BulkIndexError], retry to fix the issue")
        ...
        for _id, doc in errors.iteritems():
            ...
            if 'machine' in doc['index']['data'].keys():
                record_msg_info += ", machine [" + \
                    doc['index']['data']['machine'] + "]"
            if 'data_type' in doc['index']['data'].keys():
                record_msg_info += ", data_type[" + \
                    doc['index']['data']['data_type']+"]"
            ...
            try:
                ...
                logger.logger(level=logger.WARNING, type="elasticsearch",
                              message=record_msg_info + ": record failed", reason=doc_reason)
                ...
                if "Limit of total fields" in doc_reason and limit_fields_increased == False:
                    ...
                    inc = self.es_db.indices.put_settings(
                        index=indx, body='{"index.mapping.total_fields.limit": '+str(new_limit)+'}')

                    if inc["acknowledged"]:
                        logger.logger(level=logger.INFO, type="elasticsearch", message=record_msg_info +
                                      " : The total_fields.limit has been increased to " + str(new_limit))
                        limit_fields_increased = True
                    else:
                        logger.logger(level=logger.ERROR, type="elasticsearch",
                                      message=record_msg_info + " : failed to increase total_fields.limit")
                ...
                match = re.match(
                    'Document contains at least one immense term in field="(.+)" \(whose UTF8 encoding is longer than the max length ([0-9]+)\), all of which were skipped.* original message: bytes can be at most ([0-9]+) in length; got ([0-9]+)', doc_reason)
                if match is not None:
                    field = match.groups()[0]
                    current_max = int(match.groups()[1])
                    data_length = int(match.groups()[3])

                    logger.logger(level=logger.ERROR, type="elasticsearch", message=record_msg_info + " : field data more than the specified",
                                  reason="field " + field + ", defined max length ["+str(current_max)+"], field data ["+str(data_length)+"]")

                # ==== check if reason that an object received but the field data type is not correct
                match = re.match(
                    "object mapping for \[(.*)\] tried to parse field \[(.*)\] as (.*), but found a concrete value", doc_reason)
                if match is not None:
                    match = match.groups()
                ...
                match = re.match(
                    "failed to parse field \[(.*)\] of type \[(.*)\] in document with id .*", doc_reason)
                if match is not None:
                    match = match.groups()
                    failed_field = match[0]
                ...
                logger.logger(level=logger.ERROR, type="elasticsearch", message=record_msg_info +
                              " : No fix found for failed record ["+doc['index']['_id']+"] data", reason=doc['index']['data'])
                ...
                logger.logger(level=logger.ERROR, type="elasticsearch", message=record_msg_info +
                              " : unsuspected error in fixing record issue", reason=str(e))
                nonfixed_data.append({
                    "_index": doc['index']['_index'],
                    "_type": doc['index']['_type'],
                    "_id": doc['index']['_id'],
    ...
    def update_field(self, data, doc_id, case_id):
        try:
            index_name = self._get_index_name(case_id)
            up = self.es_db.update(
                index=index_name, doc_type="_doc", id=doc_id, body=data)
    ...
    def del_record_by_id(self, case_id, record_id):        
        try:
            index_name = self._get_index_name(case_id)
            res = self.es_db.delete(
                index=index_name, doc_type="_doc", id=record_id)
            if res['result'] == 'deleted':
                return [True, 'deleted']
            else:
                return [False, "Index["+index_name+"]: Failed to delete the record [" + str(record_id) + "]"]
    ...
    def get_mapping_fields(self, case_id):
        try:
            index_name = self._get_index_name(case_id)
            mapping = self.es_db.indices.get_mapping(index=index_name)

            if 'properties' in mapping[index_name]['mappings'].keys():
                fields = mapping[index_name]['mappings']['properties']
                fields_rec = self._get_mapping_fields_rec(fields)
    ...
    def _get_mapping_fields_rec(self, fields, current_path=[]):
        fields_list = []
        try:
            for k in fields.keys():
                if 'properties' in fields[k].keys():
                    fields_rec = self._get_mapping_fields_rec(
    ...
    def _get_index_name(self, case_id):
        if case_id.startswith(self.index_pattern):
            return case_id.lower()
        else:
            return self.index_pattern + case_id.lower()
